...
{
  emxInitStruct_struct_T7(&pStruct->workspace);
}
...
void emxCopyStruct_struct_T(i_struct_T *dst, const i_struct_T *src)
{
  ...
  emxCopy_boolean_T_8481(&dst->isActiveConstr, &src->isActiveConstr);
  emxCopy_int32_T_8481(&dst->Wid, &src->Wid);
  emxCopy_int32_T_8481(&dst->Wlocalidx, &src->Wlocalidx);
...
{
  d_emxInitStruct_anonymous_funct(&pStruct->nonlin);
  pStruct->cIneq_1.size[0] = 0;
}
...
{
  l_emxInitStruct_coder_internal_(&pStruct->next);
}
...
{
  u_emxInitStruct_coder_internal_(&pStruct->next);
}
...
/* End of code generation (mexFunctions_emxutil.c) */
