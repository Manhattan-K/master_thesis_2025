...
void initActiveSet(g_struct_T &obj)
{
  int32_T i;
  ...
  if (idxFillStart <= i) {
    std::memset(&obj.isActiveConstr.data[idxFillStart + -1], 0,
                static_cast<uint32_T>((i - idxFillStart) + 1) *
                    sizeof(boolean_T));
  }
  ...
  for (int32_T idx_local{0}; idx_local < i; idx_local++) {
    int32_T i1;
    int32_T i2;
    ...
    i1 = static_cast<uint8_T>(obj.indexFixed.data[idx_local] - 1);
    for (int32_T b_i{0}; b_i < i1; b_i++) {
      obj.ATwset[b_i + idxFillStart] = 0.0;
    }
    obj.ATwset[(obj.indexFixed.data[idx_local] + idxFillStart) - 1] = 1.0;
    i1 = obj.indexFixed.data[idx_local] + 1;
    i2 = obj.nVar;
    for (int32_T b_i{i1}; b_i <= i2; b_i++) {
      obj.ATwset[(b_i + idxFillStart) - 1] = 0.0;
    }
    obj.bwset[idx_local] = obj.ub.data[obj.indexFixed.data[idx_local] - 1];
  }
}
