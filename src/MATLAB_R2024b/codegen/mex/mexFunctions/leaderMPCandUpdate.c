...
void leaderMPCandUpdate(emlrtCTX aTLS, mexFunctionsStackData *SD,
                        ...
                        emxArray_real_T *X_L_stacked, struct6_T *b_error,
                        real_T u_t[2], real_T u_opt[40])
{
  ...
  int32_T i;
  int32_T leaderMPCandUpdate_numThreads;
  int32_T loop_ub;
  int32_T loop_ub_tmp;
  ...
  if (loop_ub_tmp < 1200) {
    ...
    leaderMPCandUpdate_numThreads = emlrtAllocRegionTLSs(
        aTLS, omp_in_parallel(), omp_get_max_threads(), omp_get_num_procs());
#pragma omp parallel for num_threads(leaderMPCandUpdate_numThreads)
