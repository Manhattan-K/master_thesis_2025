...
real_T c_maxConstraintViolation_AMats_(emlrtCTX aTLS, i_struct_T *obj,
                                       const real_T x_data[])
{
  ptrdiff_t incx_t;
  ptrdiff_t incy_t;
  ...
  real_T v;
  real_T vPrime;
  int32_T c_maxConstraintViolation_AMats__numThreads;
  int32_T i;
  int32_T idx;
  char_T TRANSA;
  v = 0.0;
  if (obj->Aineq->size[0] != 0) {
    ...
    if (i < 1200) {
      ...
      c_maxConstraintViolation_AMats__numThreads = emlrtAllocRegionTLSs(
          aTLS, omp_in_parallel(), omp_get_max_threads(), omp_get_num_procs());
#pragma omp parallel num_threads(                                              \
        c_maxConstraintViolation_AMats__numThreads) private(vPrime, d)
      {
        vPrime = rtMinusInf;
#pragma omp for nowait
        for (idx = 0; idx < i; idx++) {
          d = obj->maxConstrWorkspace->data[idx] - x_data[idx + 40];
          obj->maxConstrWorkspace->data[idx] = d;
          vPrime = muDoubleScalarMax(vPrime, d);
        }
