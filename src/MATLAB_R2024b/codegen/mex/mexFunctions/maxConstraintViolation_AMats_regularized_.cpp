...
//
// maxConstraintViolation_AMats_regularized_.cpp
//
...
real_T c_maxConstraintViolation_AMats_(g_struct_T &obj, const real_T x_data[])
{
  ptrdiff_t incx_t;
  ptrdiff_t incy_t;
  ptrdiff_t lda_t;
  ...
  if (obj.Aineq.size(0) != 0) {
    ...
    for (int32_T idx{0}; idx < i; idx++) {
      alpha1 = obj.maxConstrWorkspace[idx] - x_data[idx + 40];
...
// End of code generation (maxConstraintViolation_AMats_regularized_.cpp)
