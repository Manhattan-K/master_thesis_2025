/*
 * Academic License - for use in teaching, academic research, and meeting
 * course requirements at degree granting institutions only.  Not for
 * government, commercial, or other organizational use.
 *
 * computeQ_.c
 *
 * Code generation for function 'computeQ_'
 *
 */

/* Include files */
#include "computeQ_.h"
#include "mexFunctions_types.h"
#include "rt_nonfinite.h"
#include "blas.h"
#include "lapacke.h"
#include "omp.h"
#include <stddef.h>
#include <string.h>

/* Function Definitions */
void computeQ_(emlrtCTX aTLS, g_struct_T *obj, int32_T nrows)
{
  ptrdiff_t incx_t;
  ptrdiff_t incy_t;
  ptrdiff_t n_t;
  jmp_buf *volatile emlrtJBStack;
  int32_T computeQ__numThreads;
  int32_T i;
  int32_T iQR0;
  int32_T idx;
  int32_T obj_idx_0;
  obj_idx_0 = obj->minRowCol;
  for (idx = 0; idx < obj_idx_0; idx++) {
    int32_T n;
    iQR0 = (obj->ldq * idx + idx) + 1;
    n = (obj->mrows - idx) - 1;
    if (n >= 1) {
      n_t = (ptrdiff_t)n;
      incx_t = (ptrdiff_t)1;
      incy_t = (ptrdiff_t)1;
      dcopy(&n_t, &obj->QR->data[iQR0], &incx_t, &obj->Q->data[iQR0], &incy_t);
    }
  }
  n_t = LAPACKE_dorgqr(102, (ptrdiff_t)obj->mrows, (ptrdiff_t)nrows,
                       (ptrdiff_t)obj->minRowCol, &obj->Q->data[0],
                       (ptrdiff_t)obj->ldq, &obj->tau->data[0]);
  if ((int32_T)n_t != 0) {
    obj_idx_0 = obj->Q->size[0];
    iQR0 = obj->Q->size[1];
    obj_idx_0 *= iQR0;
    if (obj_idx_0 < 1200) {
      for (i = 0; i < obj_idx_0; i++) {
        obj->Q->data[i] = rtNaN;
      }
    } else {
      emlrtEnterParallelRegion(aTLS, omp_in_parallel());
      emlrtPushJmpBuf(aTLS, &emlrtJBStack);
      computeQ__numThreads = emlrtAllocRegionTLSs(
          aTLS, omp_in_parallel(), omp_get_max_threads(), omp_get_num_procs());
#pragma omp parallel for num_threads(computeQ__numThreads)

      for (i = 0; i < obj_idx_0; i++) {
        obj->Q->data[i] = rtNaN;
      }
      emlrtPopJmpBuf(aTLS, &emlrtJBStack);
      emlrtExitParallelRegion(aTLS, omp_in_parallel());
    }
  }
}

/* End of code generation (computeQ_.c) */
