...
void computeDeltaLag(emlrtCTX aTLS, int32_T nVar, int32_T ldJ,
                     ...
                     const emxArray_real_T *lambda, int32_T ineqL0)
{
  ...
  real_T beta1;
  int32_T b_i;
  int32_T computeDeltaLag_numThreads;
  int32_T i;
  char_T TRANSA;
  ...
  if ((uint16_T)nVar < 1200) {
    ...
    computeDeltaLag_numThreads = emlrtAllocRegionTLSs(
        aTLS, omp_in_parallel(), omp_get_max_threads(), omp_get_num_procs());
#pragma omp parallel for num_threads(computeDeltaLag_numThreads)
