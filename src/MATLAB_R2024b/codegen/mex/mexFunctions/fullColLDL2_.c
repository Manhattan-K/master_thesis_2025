...
void fullColLDL2_(m_struct_T *obj, int32_T LD_offset, int32_T NColsRemain)
{
  ptrdiff_t incx_t;
  ptrdiff_t n_t;
  real_T alpha1;
    ...
    if (!(alpha1 == 0.0)) {
      ...
      for (j = 0; j <= subMatrixDim; j++) {
        if (y != 0.0) {
          ...
          i = jA + 2;
          i1 = subMatrixDim + jA;
          scalarLB = (((((i1 - jA) + 1) / 4) << 2) + jA) + 2;
          vectorUB = scalarLB - 4;
          for (ijA = i; ijA <= vectorUB; ijA += 4) {
            __m256d r;
            r = _mm256_loadu_pd(&obj->FMat->data[ijA - 1]);
            _mm256_storeu_pd(&obj->FMat->data[ijA - 1],
                             _mm256_add_pd(r, _mm256_set1_pd(y * temp)));
          }
