                        ...
                        i_struct_T *workingset, g_struct_T *qrmanager)
{
  ...
  solution->state = 82;
  idxStartIneq_tmp =
      RemoveDependentEq_(emlrtRootTLSGlobal, memspace, workingset, qrmanager);
  if ((idxStartIneq_tmp != -1) &&
      (workingset->nActiveConstr <= qrmanager->ldq)) {
    boolean_T guard1;
    boolean_T okWorkingSet;
    RemoveDependentIneq_(emlrtRootTLSGlobal, workingset, qrmanager, memspace);
    okWorkingSet =
        feasibleX0ForWorkingSet(emlrtRootTLSGlobal, memspace->workspace_float,
                                solution->xstar.data, workingset, qrmanager);
    guard1 = false;
    if (!okWorkingSet) {
      b_RemoveDependentIneq_(emlrtRootTLSGlobal, workingset, qrmanager,
                             memspace);
      okWorkingSet =
          feasibleX0ForWorkingSet(emlrtRootTLSGlobal, memspace->workspace_float,
                                  solution->xstar.data, workingset, qrmanager);
    ...
    if (guard1 && (workingset->nWConstr[0] + workingset->nWConstr[1] ==
      ...
      constrViolation = d_maxConstraintViolation(emlrtRootTLSGlobal, workingset,
                                                 solution->xstar.data);
