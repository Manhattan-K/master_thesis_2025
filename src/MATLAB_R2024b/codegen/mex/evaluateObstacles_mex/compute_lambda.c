...
void compute_lambda(emxArray_real_T *workspace, j_struct_T *solution,
                    const d_struct_T *objective, const f_struct_T *qrmanager)
{
  ...
  if (qrmanager->ncols > 0) {
    ...
    if (guard1) {
      ...
      idxQR = (nActiveConstr_tmp / 4) << 2;
      vectorUB = idxQR - 4;
      for (idx = 0; idx <= vectorUB; idx += 4) {
        __m256d r;
        r = _mm256_loadu_pd(&workspace_data[idx]);
        _mm256_storeu_pd(&solution->lambda->data[idx],
