function [p_tp1, X_L, qi, error, u_opt] = leaderMPCandUpdate(p_t, N, optParams, obstacles, U_l_old, goal, Ts)
    ...
    costFun = @(U) leaderCost(reshape(U,2,N), p_t, ref_traj, Ts, N);
    ...
    [u_opt, ~, exitflag, output] = fmincon(costFun, U_l_old(:), [], [], [], [], lb, ub, @(U) nonLinearConst(U, p_t, qi, N, optParams.robotShape, Ts), options);
