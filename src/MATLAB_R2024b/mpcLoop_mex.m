...
%--------------------------- Obstacle avoidanace matrices -------------
    ...
    obstaclesInRange = mexFunctions('evaluateObstacles', ...
        obstacles, x_l(1:2,i), x_f(1:2,i), leaderParams, followerParams, sys, N);

%--------------------------- MPC for Leader and Follower -------------

        % MPC for the leader
    [x_l(:,i+1), X_L, X_L_stacked, ~, u_l(:,i+1), U_l_old] = mexFunctions('leaderMPCandUpdate', ...
                        sys, x_l(:,i), N, leaderParams, obstaclesInRange, U_l_old);

    X_L_plot(:,:,i) = X_L;

        % MPC for the follower
    [x_f(:,i+1), X_F, ~, u_f(:,i+1), U_f_old] = mexFunctions('followerMPCandUpdate', ...
...
%--------------------------- Feasibility-Aware Recovery Policy -------------
    ...
    if policy_halt == true
        ...
        if any(formation_error(1:k_loose_grip) > eps_loose_grip)
            ...
            [x_l(:,i+1), X_L, X_L_stacked, ~, u_l(:,i+1), U_l_old] = mexFunctions('leaderMPCandUpdateHalt', ...
...
%--------------------------- Other evaluations  ---------------------------
    ...
    leaderParams.robotShape = mexFunctions('Rmat', loadTheta)*leaderParams.initRobotShape;
    followerParams.robotShape = mexFunctions('Rmat', loadTheta)*followerParams.initRobotShape;
